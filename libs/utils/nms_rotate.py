...
def rnms_gpu(det_boxes, iou_threshold, device_id):
    if det_boxes.shape[0] == 0:
        ...
        keep = rotate_gpu_nms(det_boxes.numpy(), iou_threshold, device_id)
        keep = np.reshape(keep, [-1])
        keep = torch.from_numpy(keep, dtype=torch.int64)
        return keep
...
if __name__ == '__main__':
    ...
    keep = nms_rotate(torch.from_numpy(boxes).type(torch.float32), torch.from_numpy(scores).type(torch.float32),
                      iou_threshold=0.7, max_output_size=5)
