...
def bbox_transform_inv(boxes, deltas, scale_factors=None):
    ...
    return torch.stack([predict_xmin, predict_ymin, predict_xmax, predict_ymax], dim=1)
...
def rbbox_transform_inv(boxes, deltas, scale_factors=None):
    ...
    return torch.stack([pred_ctr_x, pred_ctr_y, pred_w, pred_h, pred_theta], dim=1)
...
def rbbox_transform_inv_dcl(boxes, deltas, scale_factors=None):
    ...
    return torch.stack([pred_ctr_x, pred_ctr_y, pred_w, pred_h], dim=1)
...
def qbbox_transform_inv(boxes, deltas, scale_factors=None):
    ...
    # pred_theta = dtheta * 180 / np.pi + boxes[:, 4]
    return torch.stack([pred_x_1, pred_y_1, pred_x_2, pred_y_2, pred_x_3, pred_y_3, pred_x_4, pred_y_4], dim=1)
...
def poly_transform_inv(boxes, deltas, point_num):
    ...
    return torch.stack(pred_xy, dim=1)
