...
def get_horizen_minAreaRectangle(boxes, with_label=True):  # done
    if with_label:
        boxes = boxes.reshape(-1, 9)
        x_list = boxes[:, :-1:2]
        y_list = boxes[:, 1:-1:2]
        label = boxes[:, -1]
        y_max, _ = y_list.max(dim=1)
        y_min, _ = y_list.min(dim=1)
        x_max, _ = x_list.max(dim=1)
        x_min, _ = x_list.min(dim=1)
        new_boxes = torch.stack([x_min, y_min, x_max, y_max, label], dim=1)
        return new_boxes
...
def coordinate5_2_8_tf(coords):
    ...
    x, y, w, h, theta = torch.split(coords, 1, dim=1)
    ...
    xlt_ = torch.cos(theta) * xlt + torch.sin(theta) * ylt + x
    ylt_ = -torch.sin(theta) * xlt + torch.cos(theta) * ylt + y

    xrt_ = torch.cos(theta) * xrt + torch.sin(theta) * yrt + x
    yrt_ = -torch.sin(theta) * xrt + torch.cos(theta) * yrt + y

    xld_ = torch.cos(theta) * xld + torch.sin(theta) * yld + x
    yld_ = -torch.sin(theta) * xld + torch.cos(theta) * yld + y

    xrd_ = torch.cos(theta) * xrd + torch.sin(theta) * yrd + x
    yrd_ = -torch.sin(theta) * xrd + torch.cos(theta) * yrd + y

    convert_box = torch.stack([xlt_, ylt_, xrt_, yrt_, xrd_, yrd_, xld_, yld_], dim=1)
    return convert_box
...
def coordinate90_2_180_tf(coords, is_radian=True, change_range=False):
    ...
    remain_mask = w > h
    convert_mask = torch.logical_not(remain_mask).type(torch.int32)
    remain_mask = remain_mask.type(torch.int32)

    remain_coords = coords * remain_mask.reshape(-1, 1).type(torch.float32)

    h_ = w.type(torch.float32)
    w_ = h.type(torch.float32)
    theta += 90.

    coords_ = torch.stack([x, y, w_, h_, theta], dim=1)

    convert_coords = coords_ * convert_mask.reshape(-1, 1).type(torch.float32)

    coords_new = remain_coords + convert_coords
    x_new, y_new, w_new, h_new, theta_new = torch.split(coords_new, 1, dim=1)
    ...
    coords_new_ = torch.stack([x_new, y_new, w_new, h_new, theta_new], dim=1)
