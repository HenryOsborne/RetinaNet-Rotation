...
class Loss(object):
    ...
    def focal_loss(self, labels, pred, anchor_state, alpha=0.25, gamma=2.0):
        ...
        per_entry_cross_ent = torch.sigmoid(F.binary_cross_entropy_with_logits(input=pred, target=labels))
        prediction_probabilities = F.sigmoid(pred)
    ...
    def iou_smooth_l1_loss_exp(self, targets, preds, anchor_state, target_boxes, anchors, sigma=3.0, alpha=1.0,
        ...
        iou_factor = tf.stop_gradient(tf.exp(alpha * (1 - overlaps) ** beta) - 1) / (
                tf.stop_gradient(regression_loss) + self.cfgs.EPSILON)
