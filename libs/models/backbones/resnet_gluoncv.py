...
class ResNetGluonCVBackbone(object):
    ...
    def basicblock_v1b(self, input_x, base_channel, scope, stride=1, projection=False, avg_down=True,
                       rm_shortcut=False):
            ...
            if projection:
                ...
                if avg_down:  # design for resnet_v1d
                    ...
                    shortcut = slim.conv2d(shortcut, num_outputs=base_channel, kernel_size=[1, 1],
                                           stride=1, padding="VALID", biases_initializer=None,
                                           data_format=self.DATA_FORMAT,
                                           activation_fn=None,
                                           scope='shortcut')
                    if self.DEBUG:
    ...
    def get_resnet_v1_b_base(self, input_x, freeze_norm, scope="resnet50_v1b", bottleneck_nums=[3, 4, 6, 3],
    ...
    def get_resnet_v1_d_base(self, input_x, freeze_norm, scope="resnet50_v1d", bottleneck_nums=[3, 4, 6, 3],
                             base_channels=[64, 128, 256, 512],
                             freeze=[True, False, False, False, False], is_training=True):
