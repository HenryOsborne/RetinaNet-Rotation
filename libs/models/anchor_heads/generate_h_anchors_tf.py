...
def generate_anchors_pre(height, width, feat_stride, anchor_scales=(8, 16, 32),
    ...
    anchors = tf.cast(tf.reshape(anchors, [1, A, 4]), tf.float32) + tf.cast(
        tf.transpose(tf.reshape(shifts, [1, K, 4]), [1, 0, 2]), tf.float32)
...
if __name__ == '__main__':
    anchors_tf = generate_anchors_pre(64, 64, 8,
                                      anchor_scales=np.array([2 ** 0, 2 ** (1.0 / 3.0), 2 ** (2.0 / 3.0)]) * 8,
